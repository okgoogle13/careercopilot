...
function canUpdateField(field) {
  return isOwner(resource.data.userId) && field in ["name", "email", "preferences"];
}
```
...
function isValidDocument() {
  return request.resource.data.keys().hasAll(["title", "content", "userId"]) && request.resource.data.title.size() <= 100;
}
```
