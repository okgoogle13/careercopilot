...
- CRITICAL EXCLUSION: No bright whites, no sepia "tea-stained" paper, no
  yellowed/aged vintage paper effects. ❌

GEOMETRIC GRID (The Engineering Layer):
- Grid Pattern: Fine 1mm scientific engineering grid.
- Controllability: Grid lines must be sharp, precise, and consistent.
- Line Color: Cool Grey (#4A4A4A) at 8-12% opacity (barely visible but present).
- Major Axes: Slightly thicker lines every 10mm to create a nested hierarchy.
- Symmetry: Perfect orthogonal alignment (mathematically vertical and horizontal).
...
SEAMLESSNESS:
- Requirement: Must tile perfectly along all four edges without visible seams.
- Pattern Balance: No large "blobs" or distinct landmarks that would create a
  noticeable "grid of patterns" when repeated.

DENSITY ZONES (UI Compatibility):
- Surface Clarity: 85% of the surface must remain "open" for UI text and elements.
- Watermark Placement: Distributed randomly but sparingly.
...
- ❌ NO "Vintage" clichés (burnt edges, ink spills, ragged corners).
