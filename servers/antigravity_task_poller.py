...
Please execute this task and provide detailed output."""
    ...
    async def _call_gemini_async(self, prompt: str, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        task_id = task['task_id']
        task_type = task['inputs'].get('task_type', 'unknown')
        output_path = task['inputs'].get('output_path') or task['inputs'].get('output_file')

        if not output_path:
            # Default to a temporary file if no output path specified
            output_path = f"/tmp/task-result-{task_id}.json"
        ...
        try:
            ...
            # Write output to file only if it's a JSON result file
            # If it's a .py file, the generator already handled it
            if output_path and output_path.endswith('.json'):
                ...
                logger.info(f"✅ Generated file should be at {output_path}")
