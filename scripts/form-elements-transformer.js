...
module.exports = function(file, api) {
    ...
    const oldImportPaths = [
        './components/ui/input', './components/ui/textarea', './components/ui/label',
        './components/ui/select', './components/ui/checkbox', './components/ui/radio-group',
        './components/ui/switch', './components/ui/slider'
    ];
    ...
    const oldImports = root.find(j.ImportDeclaration, (node) => oldImportPaths.includes(node.source.value));
    ...
    Object.keys(componentMap).forEach(oldName => {
        root.find(j.JSXIdentifier, { name: oldName }).forEach(path => {
            ...
            const isTargetComponent = binding && binding.getBindings()[oldName].some(b => oldImports.paths().includes(b.parentPath.parentPath.node));
            ...
            if (newName) {
                ...
                if (oldName === 'Input') {
                    parentElement.attributes.push(j.jsxAttribute(j.jsxIdentifier('variant'), j.literal('outlined')));
                }
                if (oldName === 'Textarea') {
                    parentElement.attributes.push(j.jsxAttribute(j.jsxIdentifier('multiline'), j.jsxExpressionContainer(j.booleanLiteral(true))));
                    parentElement.attributes.push(j.jsxAttribute(j.jsxIdentifier('rows'), j.jsxExpressionContainer(j.numericLiteral(4))));
                }
    ...
    root.find(j.JSXElement, { openingElement: { name: { name: 'Checkbox' } } }).forEach(path => {
        ...
        if (nextSibling && nextSibling.type === 'JSXElement' && nextSibling.openingElement.name.name === 'InputLabel') {
            const checkboxId = path.node.openingElement.attributes.find(a => a.name && a.name.name === 'id')?.value.value;
            const labelFor = nextSibling.openingElement.attributes.find(a => a.name && a.name.name === 'htmlFor')?.value.value;

            if (checkboxId && checkboxId === labelFor) {
                muiImportsNeeded.add('FormControlLabel');
                const labelText = j(nextSibling).find(j.JSXText).nodes().map(t => t.value.trim()).join('');

                const formControlLabel = j.jsxElement(
                    j.jsxOpeningElement('FormControlLabel', [
    ...
    // Repeat for Switch
     root.find(j.JSXElement, { openingElement: { name: { name: 'Switch' } } }).forEach(path => {
        const nextSibling = path.nextSibling;
        if (nextSibling && nextSibling.type === 'JSXElement' && nextSibling.openingElement.name.name === 'InputLabel') {
            const switchId = path.node.openingElement.attributes.find(a => a.name && a.name.name === 'id')?.value.value;
            const labelFor = nextSibling.openingElement.attributes.find(a => a.name && a.name.name === 'htmlFor')?.value.value;
    ...
    if (muiImportsNeeded.size > 0) {
        const newImport = j.importDeclaration(
            Array.from(muiImportsNeeded).sort().map(name => j.importSpecifier(j.identifier(name))),
