...
# Define I/O paths [DEPRECATED: Use scripts/build-m3-tokens.py instead]
TOKEN_SOURCE_DIR = 'frontend/src/design/tokens'
...
def generate_tailwind_patch(tokens):
    ...
    tailwind_spacing = {key.replace('space', ''): value for key, value in tokens.get('spacing', {}).get('scale', {}).items()}
    tailwind_shadows = {key.replace('_', '-'): value for key, value in tokens.get('elevation', {}).items()}
    tailwind_radius = {key.replace('_', '-'): value for key, value in tokens.get('shape', {}).items()}
    tailwind_fonts = {
        'base': [f"var(--sys-font-base)"],
        'heading': [f"var(--sys-font-heading)"],
    }
    tailwind_font_sizes = {key.replace('text', ''): value for key, value in tokens.get('typography', {}).get('scale', {}).items()}
