...
<button className="md:hidden p-2 rounded-lg">// Hamburger menu icon</button>;

{
  /* Mobile Navigation */
}
{
  isMobileMenuOpen && <div className="md:hidden py-4 border-t border-gray-700">// Collapsible mobile menu</div>;
}
```
...
<div className="hidden md:flex items-center space-x-2">// Horizontal navigation items</div>;
```
...
<div className="container mx-auto px-4">// Content with proper horizontal spacing</div>
```
...
<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3">// Responsive grid system</div>
```
