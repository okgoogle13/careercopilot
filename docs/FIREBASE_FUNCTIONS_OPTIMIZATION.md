...
export const listApplications = functions
...
```typescript
export const warmupFunctions = functions.pubsub
  .schedule('every 10 minutes')
  .onRun(async () => {
    // Ping critical functions to keep them warm
    await Promise.all([
      fetch('https://us-central1-PROJECT_ID.cloudfunctions.net/extractJobListing'),
      fetch('https://us-central1-PROJECT_ID.cloudfunctions.net/listApplications'),
    ]);
  });
```
