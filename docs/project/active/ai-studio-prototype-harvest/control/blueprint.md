...
**Version:** 1.0
**Date:** 2026-03-18
**Source of Truth:** `AI_STUDIO_HARVEST_PLAN.updated.md` (Revision 2.0)
...
Extract tokenized UI components and Python-aligned backend artefacts from the AI Studio prototype branch (`careercopilot-aistud`) into the main CareerCopilot repo. No monoliths. No hard-coded tokens. No transcription errors.
...
4. **Prototype Branch**: `careercopilot-aistud` (read-only after Phase 0 tweak completion)
...
AI Studio prompts run on `careercopilot-aistud` before main tracks begin.
...
| ID | Task | Gate |
|---|---|---|
| A-001 | Verify PT-2 output on `careercopilot-aistud` — line 1 comment + 5 snake_case field names | comment present + all names confirmed |
