# kerala-rage Image Generator Prompt Template

Use this template to generate high-fidelity, contemporary Australian assets for CareerCopilot.

## The Strategy: The Haeckel Sandwich

Every prompt must be constructed using these five layers to ensure compliance with the kerala-rage kr-solidarity aesthetic.

### 1. The Base: Absolute Void

- **Requirement**: Background must be **Asphalt Black #1A1714**.
- **Description**: A deep, nocturnal urban charcoal. Not pure black, not sepia.
- **Keywords**: "Absolute nocturnal void", "Asphalt Black #1A1714 background", "Melbourne laneway atmosphere".

### 2. The Filling: Specific Subject

- **Requirement**: **Australian Endemic Species** only.
- **Living Context**: The species must be in an active, living scene (e.g., on power lines, street signs, urban gardens).
- **Species**: [SPECIES OR SUBJECT]
- **Prohibited**: No "kr-motifs", no "museum cabinets", no "pinning", no "preservation".

### 3. The Topping: Geometric Lens & Surgical Light

- **Geometry**: Underlying Haeckelian order (radial symmetry, Fibonacci spirals, hexagonal lattices).
- **Lighting**: "Surgical Light" — sharp, dramatic rim lighting or studio-style chiaroscuro to separate the subject from the void.
- **Perspective**: Orthographic or precise isometric views preferred.

### 4. The Seasoning: Restricted Palette

- **Primary**: Wattle Gold `#D4A84B`
- **Secondary**: Waratah Red `#C45C4B`
- **Neutral**: Paper White `#F5F0E8` (Use for annotations/highlights)
- **Accent**: Gum Leaf Green `#6B7F6E`

### 5. The Five Immutable Laws

1.  **Contemporary, Not Nostalgic**: No sepia, no vintage filters, no kerala-streetprint fonts.
2.  **Graphic Contrast**: Bold silhouettes and high-contrast edges.
3.  **Anatomical Extremism**: Capture intense biological detail (skeletal structures, microscopic lattices).
4.  **Peter Drew Aesthetic**: Poster-like boldness, unapologetic silhouettes.
5.  **Alpha Transparency Ready**: Ensure the subject "melts" into the #1A1714 background for clean UI integration.

---

## The Prompt Template

**Template Copy/Paste:**

> A contemporary, bold [ASSET TYPE] of [SPECIES OR SUBJECT].
>
> **Composition**: [TYPE-SPECIFIC BLOCK]
>
> **Aesthetic**: Peter Drew street art influence, high-contrast graphic silhouette. Extreme anatomical detail.
> **Atmosphere**: The subject is positioned within an absolute nocturnal void of Asphalt Black (#1A1714).
> **Lighting**: Surgical top-down rim lighting in Wattle Gold (#D4A84B) and Waratah Red (#C45C4B) highlights.
> **Palette**: Strictly restricted to Asphalt Black (#1A1714), Paper White (#F5F0E8), Wattle Gold (#D4A84B), and [ACCENT COLOR].
>
> **Forbidden**: vintage, sepia, kr-motif, museum, cabinet, naturalist, federaton, colonial, cute, soft.
>
> **Negative Prompt**: sepia, vintage, low-res, blurry, cute, cartoon, 3d render, museum cabinet, kr-motif plate.

---

## Asset Type Blocks

### HERO

- **Focal Tier**: Primary (12-20cm focal subject).
- **Framing**: Off-center (Rule of Thirds), leaving negative space for typography.
- **Mood**: Emotional, guardian-like, bold.

### UI ACCENT

- **Focal Tier**: Secondary/Tertiary (2-12cm).
- **Framing**: Centered or side-cropped, extremely clean edges.
- **Mood**: Precise, geometric, functional.

### BACKGROUND

- **Focal Tier**: Fractal/Textural.
- **Framing**: Edge-to-edge pattern or subtle corner growth.
- **Mood**: Atmospheric, low-density, secondary.
