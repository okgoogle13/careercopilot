...
export function ProgressStepper({
    ...
    return (
      ...
      >
        ...
        <div className={cn(
          ...
          {status === 'completed' ? (
            <Check className={cn('text-on-primary', variant === 'compact' ? 'w-4 h-4' : 'w-6 h-6')} />
          ) : step.icon ? (
            <div className={cn(variant === 'compact' ? 'w-4 h-4' : 'w-6 h-6')}>
              {step.icon}
            </div>
          ) : (
        ...
        {variant !== 'compact' && (
          <div className={cn(
            ...
            <div className={cn(
              ...
              {step.title}
              {step.optional && (
                <span className="text-xs text-secondary ml-1">(Optional)</span>
              )}
            </div>
  ...
  const StepConnector = ({ index }: { index: number }) => {
    ...
    const isCompleted = isStepCompleted(index) && (isStepCompleted(index + 1) || isStepCurrent(index + 1));
  ...
  return (
    <div className={cn('bg-surface-container rounded-lg p-4', className)}>
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-sm text-muted-foreground">{completed}/{totalSteps} completed</span>
      </div>
