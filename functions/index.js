...
// Set global default region for all functions
...
exports.cleanupUserData = onUserDeleted(async (event) => {
  ...
  try {
    ...
    logger.log(`Successfully deleted Firestore data for user: ${uid}`);
  } catch (error) {
  ...
  try {
    ...
    logger.log(`Successfully deleted Cloud Storage files for user: ${uid}`);
  } catch (error) {
    // It's possible the user has no files, so we check for specific errors.
    if (error.code === 404) {
      logger.log(`No Cloud Storage files to delete for user: ${uid}`);
    } else {
      logger.error(`Error deleting Cloud Storage files for user ${uid}:`, error);
    }
  }
