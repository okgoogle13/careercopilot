...
def main():
    ...
    if not DRY_RUN and BACKUP:
        ...
        try:
            shutil.copytree(ROOT_DIR, BACKUP_DIR, ignore=shutil.ignore_patterns(
                'node_modules', '.venv', '.git', '__pycache__', 'dist', 'build', 'out', '.next', '.cache',
                'secret_scan_raw.txt', '*.zip', '*.mov', '*.mp4', '*.dmg', '*.iso', '*.tar.gz'
            ))
    ...
    # 2b. Special handling for tools/ -> tools/admin/
    ...
    if tools_dir.exists():
        ...
        start_items = list(tools_dir.iterdir())
