...
class PersonalCareerConfig:
    ...
    career_transition_to: str = os.getenv(
        "CAREER_TRANSITION_TO", "Social Work/Community Services"
    )
    career_motivation: str = os.getenv(
        "CAREER_MOTIVATION", "Direct community impact and social justice"
    )
    ...
    work_types: List[WorkType] = field(
        default_factory=lambda: [WorkType.FULL_TIME, WorkType.PART_TIME]
    )
    ...
    developing_skills: List[str] = field(
        default_factory=lambda: [
            "Case Management",
    ...
    current_qualifications: List[str] = field(
        default_factory=lambda: [
    ...
    target_qualifications: List[str] = field(
        default_factory=lambda: [
    ...
    preferred_org_types: List[str] = field(
        default_factory=lambda: [
    ...
    organization_values: List[str] = field(
        default_factory=lambda: [
            "Social Justice",
            "Cultural Diversity",
    ...
    auto_generate_documents: bool = bool(
        os.getenv("AUTO_GENERATE_DOCUMENTS", "true").lower() == "true"
    )
    email_notifications: bool = bool(
        os.getenv("EMAIL_NOTIFICATIONS", "true").lower() == "true"
    )
    ...
    job_sources: Dict[str, str] = field(
        default_factory=lambda: {
            "seek": os.getenv(
                "SEEK_RSS_URL", "https://www.seek.com.au/jobs-in-social-work/rss"
            ),
            "indeed": os.getenv(
                "INDEED_RSS_URL", "https://au.indeed.com/rss?q=social+work+melbourne"
            ),
            "ethical_jobs": os.getenv(
                "ETHICAL_JOBS_RSS",
                "https://www.ethicaljobs.com.au/rss/jobs?category=social-services",
            ),
    ...
    document_preferences: Dict[str, str] = field(
        default_factory=lambda: {
    ...
    def calculate_job_match_score(
        self, job_title: str, job_description: str, company_type: str = ""
    ) -> float:
        ...
        skill_matches = sum(
            1 for skill in self.transferable_skills if skill.lower() in job_text
        )
        ...
        # Values alignment
        values_matches = sum(
            1 for value in self.organization_values if value.lower() in job_text
        )
