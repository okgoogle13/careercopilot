  ...
  <div key={app.id} className="bg-[#25232A] rounded-[28px] p-8">
    ...
    <div className="flex gap-2">
      {app.steps.map((step, idx) => {
        ...
        return (
          <div key={idx} className={`flex-1 px-4 py-3 rounded-full text-center transition-all ${
            isCurrent ? 'bg-[#D0BCFF] text-[#381E72]' : 
            isCompleted ? 'bg-[#A8C5A3] text-[#141218]' : 
...
{applications.map((app) => (
  <ApplicationCard
    ...
    steps={app.steps}
    onUpdateStatus={() => console.log('Update')}
  />
))}
```
...
<motion.button
  ...
  whileTap={{ scale: 0.98 }}
  className="bg-[var(--surface-container-high)] text-[var(--primary-sage)] py-4 px-8 rounded-full hover:bg-[var(--surface-bright)] transition-colors font-semibold uppercase tracking-wider flex items-center gap-3"
>
...
```tsx
<button className="bg-black/50 backdrop-blur-[10px] border border-white/5 shadow-[inset_0_0_12px_rgba(255,255,255,0.05)] py-4 px-8 rounded-full hover:bg-black/60 transition-all">
  View Analytics
</button>
```
...
```tsx
<button className="bg-[var(--surface-container-high)] text-[var(--primary-sage)] py-4 px-8 rounded-full hover:bg-[var(--surface-bright)] transition-colors">
  CONNECT
</button>
```
...
**3. Outlined** (Tertiary)
```tsx
<button className="bg-transparent border-2 border-[#CAC4D0]/30 py-4 px-8 rounded-full hover:border-[#D0BCFF]/50 transition-all">
  + Add New
</button>
```
