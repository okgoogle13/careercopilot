...
function fixContent(src: string) {
  ...
  out = out.replace(/(:\s*)([0-9]+(?:\.[0-9]+)?)\s*(px|rem)(\s*[,}])/g, (_m, p1, num, unit, tail) => `${p1}"${num}${unit}"${tail}`);
  ...
  out = out.replace(/(typography\s*:\s*)(body1|body2|h1|h2|h3|h4|h5|h6)(\b)/g, (_m, p1, tok, p3) => `${p1}"${tok}"${p3}`);
