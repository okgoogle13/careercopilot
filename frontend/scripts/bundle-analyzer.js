...
function analyzeBundle() {
  ...
  if (totals.javascript > SIZE_LIMITS.javascript) {
    warnings.push(
      `JavaScript bundle size (${formatBytes(totals.javascript)}) exceeds limit (${formatBytes(SIZE_LIMITS.javascript)})`
    );
  }
  if (totals.css > SIZE_LIMITS.css) {
    warnings.push(
      `CSS bundle size (${formatBytes(totals.css)}) exceeds limit (${formatBytes(SIZE_LIMITS.css)})`
    );
  }
  if (totals.total > SIZE_LIMITS.total) {
    warnings.push(
      `Total bundle size (${formatBytes(totals.total)}) exceeds limit (${formatBytes(SIZE_LIMITS.total)})`
    );
  }
  ...
  console.log(
    `🟨 JavaScript: ${analysis.totals.javascriptFormatted} (${assets.javascript.length} files)`
  );
