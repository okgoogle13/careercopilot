...
export function useAnalysis() {
  ...
  const analyzeDocument = useCallback(
    async (documentText: string, jobCriteria: string, useAI = false): Promise<AnalysisResult> => {
      setAnalyzing(true);
      ...
      try {
        ...
        const missingKeywords = criteriaKeywords.filter((kw) => !matchedKeywords.includes(kw));
        ...
        const overall = Math.round(
          hardSkillsScore * 0.3 + softSkillsScore * 0.2 + impactScore * 0.3 + atsScore * 0.2
        );
