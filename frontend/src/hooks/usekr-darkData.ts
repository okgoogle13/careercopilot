...
export type kr-darkFeedItemType = 'opportunity' | 'insight' | 'alert';

export interface kr-darkFeedItem {
    id: string;
    type: kr-darkFeedItemType;
    title: string;
    timestamp: string;
    description: string;
    meta?: {
...
export function usekr-darkData() {
    const fallback = useMemo<kr-darkFeedItem[]>(
        () => [
            {
                id: 'feed-1',
                type: 'opportunity',
                ...
                meta: { company: 'kerala-rage Labs', matchScore: 92 },
            },
            {
                id: 'feed-2',
                type: 'insight',
    ...
    const [feed, setFeed] = useState<kr-darkFeedItem[]>(fallback);
