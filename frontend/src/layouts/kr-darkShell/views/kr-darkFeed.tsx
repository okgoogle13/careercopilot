...
import { GlassLeafCard } from '@/features/kr-dark/GlassLeafCard';
import { usekr-darkData } from '@/hooks/usekr-darkData';
import { motion } from 'framer-motion';
import { Briefcase, Sparkles, TrendingUp } from 'lucide-react';
import React from 'react';

export const kr-darkFeed: React.FC = () => {
  const { feed, isLoading } = usekr-darkData();
      ...
      >
        <div className="flex items-baseline justify-between mb-8">
          ...
          <div className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-status-kr-dark-alive-green animate-pulse" />
                ...
                >
                  <GlassLeafCard className="flex items-start gap-6 group">
                    ...
                    <div className="shrink-0 relative">
                      <div className="w-12 h-12 rounded-full bg-surface-kr-dark-glass-medium border border-white/10 flex items-center justify-center text-wattle-gold">
                        {getIcon(item.type)}
                      </div>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-start">
                        <div>
                          <h3 className="text-bloom-kr-dark text-xl transition-colors">
                            {item.title}
                          </h3>
                    ...
                    {/* Decorative kr-motif Asset Injection (Randomized for demo) */}
                    {idx % 2 === 0 && (
                      <div className="absolute -right-8 -top-8 w-32 h-32 opacity-10 pointer-events-none group-hover:opacity-20 transition-opacity duration-700 mix-blend-overlay">
                        <img
                          src="/src/assets/kr-motifs/leaf-fern.png"
                          alt=""
                    ...
                    {idx % 2 !== 0 && (
                      <div className="absolute -right-4 bottom-4 w-24 h-24 opacity-5 pointer-events-none group-hover:opacity-15 transition-opacity duration-700 mix-blend-overlay">
                        <img
                          src="/src/assets/kr-motifs/beetle-scarab.png"
                          alt=""
                          className="w-full h-full object-contain transform -rotate-12"
                        />
                      </div>
