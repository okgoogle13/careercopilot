...
export const GalleryFeed: React.FC = () => {
    ...
    const getIcon = (type: string) => {
        switch (type) {
            ...
            case 'insight': return <Sparkles className="w-5 h-5" />;
            case 'alert': return <TrendingUp className="w-5 h-5" />;
            ...
            >
                ...
                <div className="space-y-6">
                    {isLoading ? (
                        /* Skeletons */
                        [1, 2, 3].map(i => (
                            <div key={i} className="h-28 bg-surface-specimen-night-light border border-secondary-flannel-dim/20 rounded-[var(--radius-stone)] animate-pulse" />
                        ))
                    ) : (
                        feed.map((item, idx) => (
                            <motion.div
                                ...
                                <GlassLeafCard className="flex items-start gap-6 group">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <div className="mt-4 flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity duration-300 transform translate-y-2 group-hover:translate-y-0">
                                            <Pebble variant="secondary" size="sm">Dismiss</Pebble>
                                            <Pebble variant="primary" size="sm">Investigate</Pebble>
                                        </div>
                                    </div>
