...
export function KscGeneratorPage({
  ...
  const [criteria, setCriteria] = useState<KscCriterion[]>([
    ...
    },
    {
      ...
      description: 'Ability to work effectively in cross-functional teams and communicate technical concepts',
      category: 'essential',
      isCompleted: false,
      suggestedLength: 250,
      examples: [
    ...
    setTimeout(() => {
      setCriteria(prev => prev.map(c =>
        c.id === criterionId
          ? {
              ...c,
              response: 'Generated response based on your profile and the job requirements. This response demonstrates relevant experience and skills that align with the specified criterion.',
              wordCount: 280,
              isCompleted: true
            }
          : c
              ...
              <Box sx={{ p: 4 }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  ...
                  <Stack direction="row" spacing={2}>
                    ...
                    <Button
                      variant="contained"
                      startIcon={extracting ? <CircularProgress size={20} color="inherit" /> : <AutoAwesome />}
          ...
          {activeStep >= 1 && (
            <Paper elevation={0} sx={{ border: 1, borderColor: 'divider', borderRadius: 2 }}>
                            ...
                            <Box sx={{ mb: 2 }}>
                              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
                                ...
                                <Stack direction="row" spacing={1}>
                                  <Button
                                    size="small"
                                    startIcon={generating && selectedCriterion === criterion.id ?
                                      <CircularProgress size={16} color="inherit" /> : <SmartToy />}
                                    onClick={() => handleGenerateResponse(criterion.id)}
                                    disabled={generating}
                                  >
                                    {generating && selectedCriterion === criterion.id ? 'Generating...' : 'AI Generate'}
                                  </Button>
                              ...
                              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 1 }}>
                                ...
                                {criterion.wordCount && (
                                  <Chip
                                    ...
                                    color={getWordCountColor(criterion.wordCount, criterion.suggestedLength) as any}
                                  />
                                )}
                              </Box>
                            </Box>
              ...
              <TabPanel value={tabValue} index={1}>
                <Box sx={{ p: 3 }}>
                  {criteria.filter(c => c.category === 'essential').map((criterion) => (
              ...
              <TabPanel value={tabValue} index={2}>
                <Box sx={{ p: 3 }}>
                  {criteria.filter(c => c.category === 'desirable').map((criterion) => (
                    // Same accordion structure as above
            ...
            <CardContent>
              ...
              <Stack spacing={2}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <Typography variant="body2" fontWeight={600}>
                    {Math.max(1, Math.ceil((criteria.length - criteria.filter(c => c.isCompleted).length) * 15))} min
                  </Typography>
                </Box>
              </Stack>
            </CardContent>
