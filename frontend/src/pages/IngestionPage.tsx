...
export const IngestionPage: React.FC = () => {
    ...
    const getStageMessage = (): string => {
        switch (uploadStage) {
            case 'uploading': return 'Uploading payloads...';
            case 'extracting': return 'Harvesting semantic text...';
            case 'processing': return 'Generating tactical insights...';
            case 'embedding': return 'Mapping professional vector...';
            case 'complete': return 'Synthesis complete!';
            default: return '';
        }
    };
        ...
        <div className="min-h-screen bg-[var(--color-specimen-night)] flex items-center justify-center p-6 bg-gradient-to-b from-transparent to-[var(--color-wattle-gold)]/5">
            ...
            >
                ...
                <header className="text-center mb-10">
                    ...
                    <h1 className="font-bloom text-4xl font-bold text-[var(--color-parchment)] tracking-tight">
                        Career DNA Ingestion
                    </h1>
                    <p className="font-field-note text-lg text-[var(--color-flannel-flower-dark)] mt-2">
                ...
                <div
                    className={`
                        border-2 border-dashed rounded-[var(--radius-pebble)] p-10 text-center mb-8
                    ...
                    {selectedFiles.length > 0 && (
                        <div className="mt-8 space-y-3">
                            {selectedFiles.map((file, idx) => (
                                <div key={idx} className="flex items-center gap-3 p-3 bg-white/5 rounded-[var(--radius-stone)] border border-white/5">
                                    <FileText className="w-5 h-5 text-[var(--color-wattle-gold)]" />
                                    <div className="text-left flex-grow">
                                        <div className="text-sm font-field-note text-[var(--color-parchment)]">{file.name}</div>
