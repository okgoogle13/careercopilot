            ...
            <div className="max-w-6xl mx-auto">
                ...
                <header className="flex flex-col md:flex-row justify-between items-start gap-8 mb-12">
                    <div>
                        ...
                        <p className="font-field-note text-xl text-[var(--color-flannel-flower-dark)] mt-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                    ...
                    >
                        <AlertTriangle className={`w-10 h-10 ${flaggedAchievements.length + flaggedKSCs.length > 0 ? 'text-orange-500' : 'text-[var(--color-flannel-flower-dark)]/40'}`} />
                        <div>
                            <span className="font-bloom text-3xl font-bold text-[var(--color-parchment)] block">
                ...
                <div className="space-y-6">
                    ...
                    >
                        <div className="space-y-8 mt-6">
                            {localData.Structured_Achievements.map((achievement: StructuredAchievement, idx: number) => (
                                <div key={achievement.Achievement_ID} className="relative pl-6 border-l border-white/10 pb-8 last:pb-0">
                                    ...
                                    <div className="flex justify-between items-center mb-6">
                                        <span className="font-annotation text-[10px] text-[var(--color-flannel-flower-dark)] uppercase tracking-widest">
                                            Evidence Node {idx + 1}
                                        </span>
                        ...
                        >
                            <div className="space-y-12 mt-8">
                                {localData.KSC_Responses.map((ksc: KSCResponse, idx: number) => (
                                    <div key={ksc.KSC_ID} className="bg-white/5 rounded-[var(--radius-pebble)] p-8 border border-white/5">
                                        <div className="flex justify-between items-start mb-8">
                                            <div className="flex-1 mr-4">
                                                <span className="font-annotation text-[10px] text-[var(--color-wattle-gold)] uppercase tracking-widest block mb-2">Module {idx + 1} Prompt</span>
                                                <h4 className="font-bloom text-lg text-[var(--color-parchment)] italic">"{ksc.KSC_Prompt}"</h4>
                                            </div>
                                            <StatusChip needsReview={ksc.Needs_Review_Flag} />
                                        </div>

                                        {ksc.Needs_Review_Flag && ksc.STAR_Feedback && (
                                            <div className="bg-orange-500/10 border border-orange-500/20 rounded-[var(--radius-stone)] p-4 mb-8 flex gap-4">
                                                <BrainCircuit className="w-5 h-5 text-orange-500 shrink-0" />
                                                <div className="text-xs font-field-note text-[var(--color-parchment)]/80 leading-relaxed">
                                                    <span className="font-bold text-orange-500 uppercase tracking-tighter block mb-1">AI Tactical Feedback</span>
                                                    {ksc.STAR_Feedback}
                                                </div>
                                            </div>
                                        )}
                ...
                <footer className="mt-20 pt-10 border-t border-white/5 flex justify-center pb-20">
                    <div className="flex items-center gap-3 px-6 py-3 bg-[var(--color-eucalypt-smoke-base)]/10 text-[var(--color-eucalypt-smoke-base)] rounded-full text-xs font-annotation uppercase tracking-[0.2em] border border-[var(--color-eucalypt-smoke-base)]/20 shadow-lg">
