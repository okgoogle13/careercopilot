        ...
        <div className="p-8 bg-surface-container rounded-xl border border-outline-variant space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                    <p className="text-sm text-on-surface-variant font-mono">Current State: {state.toUpperCase()}</p>
                    <motion.div
                    ...
                    <div className="flex gap-2">
                        {(['pebble', 'leaf', 'gem'] as const).map(shape => (
                            <button
                                ...
                                className={`px-3 py-1 rounded-full text-sm font-bold uppercase transition-all ${currentShape === shape
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-center">
                            ...
                            <span className={`text-lg font-bold ${axes.wght === restAxes.wght ? 'text-green-500' : 'text-yellow-500'}`}>
                                {axes.wght}
                            </span>
                        </div>
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-center">
                            ...
                            <span className={`text-lg font-bold ${isHovering ? 'text-secondary' : 'text-on-surface'}`}>
                                {axes.wdth}
                            </span>
                        </div>
                        <div className="h-2 bg-surface-dim rounded-full overflow-hidden">
                    ...
                    <div className="space-y-2">
                        <div className="flex justify-between items-center">
                            ...
                            <span className={`text-lg font-bold ${isHovering ? 'text-tertiary' : 'text-on-surface'}`}>
                                {axes.GRAD}
                            </span>
                        </div>
                        <div className="h-2 bg-surface-dim rounded-full overflow-hidden">
    ...
    useEffect(() => {
        ...
        const observer = new ResizeObserver((entries) => {
            for (const entry of entries) {
                ...
                if (Math.abs(width - initialSizeRef.current.width) > 2 ||
    ...
    return (
        <div className="p-8 bg-surface-container rounded-xl border border-outline-variant space-y-6">
            ...
            <p className="text-sm text-on-surface-variant">
                ...
                <strong className="text-on-surface"> Hover the test element while monitoring to see if it triggers layout shifts.</strong>
            </p>
            ...
            <div className="flex gap-4">
                <button
                    ...
                    className={`px-4 py-2 rounded-lg font-bold text-sm ${testType === 'correct'
                ...
                <button
                    ...
                    className={`px-4 py-2 rounded-lg font-bold text-sm ${testType === 'wrong'
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="h-64 bg-gradient-to-br from-warning/10 to-error/10 rounded-xl flex items-center justify-center border-2 border-dashed border-warning/30">
                    {testType === 'correct' ? (
                        ...
                        >
                            ...
                            <div className="text-sm text-on-surface-variant mt-2">Changes font-weight (causes reflow)</div>
                        </motion.div>
                    )}
                </div>
                ...
                {/* Violations Display */}
                <div className="space-y-4">
                    <div className={`p-4 rounded-lg border-2 ${violations.length > 0
                        ...
                        {violations.length > 0 && (
                            <div className="space-y-1 mt-3">
                                {violations.map((v, i) => (
                                    <div key={i} className="text-xs font-mono text-error bg-error/5 p-2 rounded">
                                        {v}
                                    </div>
                                ))}
                            </div>
                    ...
                    <div className="bg-surface-dim p-4 rounded-lg">
                        ...
                        <p className="text-xs text-on-surface-variant leading-relaxed">
                            <code className="text-error">font-weight</code> must remain <strong>constant</strong> during morphs.
                            Use <code className="text-green-500">GRAD</code> and <code className="text-green-500">wdth</code> axes instead.
                            This prevents expensive layout recalculations and visual jank.
                        </p>
                    </div>
                </div>
            </div>
