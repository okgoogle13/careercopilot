...
const ATSScoreCircle = ({ score, size }: { score: number; size?: string }) => (
  <div
    className={`p-2 rounded-march border-2 border-primary flex items-center justify-center font-bold ${size === 'small' ? 'w-10 h-10 text-xs' : 'w-20 h-20 text-lg'}`}
  >
    {score}
  </div>
);
      ...
      <Card className="p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {leftProfile.skills.map((skill, index) => (
            <Badge
              key={index}
              variant={!rightProfile.skills.includes(skill) ? 'destructive' : 'default'}
            >
              {skill}
            </Badge>
          ))}
      ...
      <Card className="p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {rightProfile.skills.map((skill, index) => (
            <Badge
              key={index}
              variant={!leftProfile.skills.includes(skill) ? 'secondary' : 'default'}
            >
              {skill}
            </Badge>
          ))}
    ...
    <div className="w-full p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
        <div>
          ...
          <Card className="p-6">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-16 h-16 rounded-march bg-primary-container flex items-center justify-center text-on-primary-container text-xl font-bold">
                {leftProfile.name[0]}
              </div>
              <div>
        ...
        <div>
          ...
          <Card className="p-6">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-16 h-16 rounded-march bg-secondary-container flex items-center justify-center text-on-secondary text-xl font-bold">
                {rightProfile.name[0]}
              </div>
              <div>
                <h4 className="text-hero text-lg font-semibold text-on-surface">
