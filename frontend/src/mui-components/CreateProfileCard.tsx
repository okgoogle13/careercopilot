...
export const CreateProfileCard: React.FC<CreateProfileCardProps> = ({ onCreate }) => {
  return (
    <Card onClick={onCreate} sx={{ height: '100%', display: 'flex', flexDirection: 'column', cursor: 'pointer', borderStyle: 'dashed', borderWidth: 2, borderColor: (theme) => alpha(theme.palette.primary.main, 0.3), bgcolor: (theme) => alpha(theme.palette.primary.main, 0.04), '&:hover': { borderColor: 'primary.main', bgcolor: (theme) => alpha(theme.palette.primary.main, 0.08) } }}>
      <CardContent sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', textAlign: 'center', gap: 3, p: 4 }}>
        <Box sx={{ position: 'relative' }}>
          <Box sx={{ width: 64, height: 64, borderRadius: 3, bgcolor: (theme) => alpha(theme.palette.primary.main, 0.12), display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'transform 0.3s' }}>
            <AddIcon sx={{ fontSize: 32, color: (theme) => theme.palette.primary.main }} />
          </Box>
          <Box sx={{ position: 'absolute', top: -4, right: -4, width: 24, height: 24, borderRadius: '50%', background: (theme) => `linear-gradient(135deg, ${theme.palette.primary.main}, ${theme.palette.secondary.main})`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
