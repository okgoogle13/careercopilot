# SettingsPageShell Spec

Source wireframe: `10_settings.xml`
Wireframe title: **Settings & Profile Management**

## 1) TypeScript Interface

```ts
export interface SettingsPageShellViewModel {
  title: string;
  subtitle?: string;
  primaryLabel?: string;
...
export interface SettingsPageShellProps {
  viewModel?: SettingsPageShellViewModel; // Optional data model for configurable content
...
|---|---|---|---|
| `settings-menu` | `navigation` | `navigation` | `settings-menu` |
| `settings-profile-title` | `text` | `heading` | `section-title` |
| `settings-name` | `input` | `textbox` | `form-field` |
| `settings-email` | `input` | `textbox` | `form-field` |
| `settings-pref-title` | `text` | `heading` | `section-title` |
| `settings-notify-email` | `toggle` | `switch` | `preference-toggle` |
| `settings-match-frequency` | `select` | `combobox` | `form-field` |
| `settings-save` | `button` | `button` | `primary-cta` |
| `settings-reset` | `button` | `button` | `secondary-cta` |
...
describe('SettingsPageShell', () => {
  it('renders core screen structure', () => {
    render(<SettingsPageShell viewModel={{ title: 'Example' }} />);
    expect(screen.getByRole('main')).toBeInTheDocument();
  });

  it('fires primary action callback', async () => {
    const onPrimaryAction = vi.fn();
    render(<SettingsPageShell viewModel={{ title: 'Example' }} onPrimaryAction={onPrimaryAction} />);
  ...
  it('announces error state accessibly', () => {
    render(<SettingsPageShell viewModel={{ title: 'Example' }} errorMessage="Failure" />);
...
- Suggested `data-testid` seeds: `settings-email`, `settings-match-frequency`, `settings-menu`, `settings-name`, `settings-notify-email`, `settings-pref-title`, `settings-profile-title`, `settings-reset`, `settings-save`
