...
const PageLoading = memo<PageLoadingProps>(({ message = 'Loading page...', className = '' }) => (
  <div
...
const InlineLoading = memo<InlineLoadingProps>(({ message = 'Loading...', className = '' }) => (
...
const TableLoading = memo<TableLoadingProps>(({ rows = 5, columns = 4, className = '' }) => (
  <div className={`animate-pulse ${className}`} role='status' aria-label='Loading table data'>
    {Array.from({ length: rows }).map((_, rowIndex) => (
      <div key={rowIndex} className='flex space-x-4 py-3 border-b border-border'>
...
const CardLoading = memo<CardLoadingProps>(({ className = '', variant = 'default' }) => (
  <div
