...
const ErrorHandlingDemo: React.FC = () => {
  ...
  const simulateNetworkError = () => {
    const error = createError.network('Failed to connect to the server', {
      component: 'ErrorHandlingDemo',
      attemptedUrl: '/api/test',
    });
    addError(error);
  ...
  const simulateValidationError = () => {
    const error = createError.validation('Please fill in all required fields', {
      component: 'ErrorHandlingDemo',
  ...
  const simulateAuthError = () => {
    const error = createError.authentication('Your session has expired', {
      component: 'ErrorHandlingDemo',
      sessionId: 'abc123',
    });
    addError(error);
  ...
  const simulateAPIError = () => {
    const error = createError.api('The server returned an error', 500, {
      component: 'ErrorHandlingDemo',
  ...
  const simulateFileError = () => {
    const error = createError.fileUpload('File is too large or in an unsupported format', {
      component: 'ErrorHandlingDemo',
      fileName: 'resume.pdf',
      fileSize: '15MB',
      maxSize: '10MB',
  ...
  const simulateAIError = () => {
    const error = createError.ai('AI service is temporarily unavailable', {
      component: 'ErrorHandlingDemo',
  ...
  return (
    <div className='p-6 max-w-4xl mx-auto space-y-8'>
      <div className='space-y-4'>
        <h2 className='text-2xl font-bold text-gray-900'>Error Handling System Demo</h2>
        <p className='text-gray-600'>
          This demo shows how the comprehensive error handling system works with actionable
      ...
      <div className='bg-white p-6 rounded-lg border space-y-4'>
        ...
        <div className='flex space-x-4'>
          <Button onClick={handleAsyncOperation}>Test Async Operation</Button>
          <Button onClick={testAPIWrapper}>Test API Wrapper</Button>
        </div>
      </div>
      ...
      {currentError && (
        <div className='bg-white p-6 rounded-lg border space-y-4'>
          <div className='flex justify-between items-center'>
            ...
            <Button onClick={attemptRecovery} disabled={isRecovering} size='sm'>
          ...
          <div className='space-y-2'>
            {recoveryAttempts.slice(0, 5).map((attempt, index) => (
              <div key={index} className='flex justify-between items-center p-3 bg-gray-50 rounded'>
                <span className='text-sm'>Strategy: {attempt.strategy}</span>
                <span
      ...
      <div className='bg-white p-6 rounded-lg border space-y-4'>
        <div className='flex justify-between items-center'>
          <h3 className='text-lg font-semibold'>Global Error Queue ({errors.length})</h3>
          {errors.length > 0 && (
            ...
            {errors.map(error => (
              <div key={error.id} className='p-3 bg-gray-50 rounded text-sm'>
                ...
                <div className='text-xs text-gray-500 mt-1'>
                  Component: {error.component} | Severity: {error.severity} | Time:{' '}
