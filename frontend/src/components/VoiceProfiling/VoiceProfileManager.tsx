...
const VoiceProfileManager: React.FC<VoiceProfileManagerProps> = ({
  ...
  const analyzeText = async (
    text: string,
    type: 'resume' | 'cover_letter' | 'email' | 'social' = 'email'
  ) => {
    if (!text.trim()) {
      ...
      if (response.ok) {
        ...
        if (profile) {
          setProfile({
            ...
            vocabulary: analysisResult.vocabulary as
              | 'simple'
              | 'technical'
              | 'business'
              | 'academic'
              | 'mixed',
            sentenceStructure: analysisResult.sentenceStructure as
              | 'short'
              | 'medium'
              | 'long'
              | 'varied',
            commonPhrases: [...profile.commonPhrases, ...analysisResult.keyPhrases.slice(0, 5)],
            sampleTexts: [...profile.sampleTexts, { type, content: text, analyzed: true }],
  ...
  return (
    <div className='max-w-6xl mx-auto space-y-6'>
      ...
      <div className='min-h-96'>
        {activeTab === 'setup' && <ProfileSetupTab profile={profile} onUpdate={updateProfile} />}
        ...
        {activeTab === 'preview' && (
          <PreviewTab profile={profile} onGenerateSample={generateSampleContent} />
        )}
      </div>
          ...
          <div className='space-y-4'>
            ...
            <div>
              ...
              <select
                ...
                onChange={e =>
                  onUpdate({
                    sentenceStructure: e.target.value as 'short' | 'medium' | 'long' | 'varied',
                  })
                }
                className='w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500'
              >
        ...
        <Card className='p-6'>
          ...
          <div className='space-y-4'>
            ...
            <div className='space-y-3'>
              <label className='block text-sm font-medium text-gray-700'>Writing Preferences</label>
              ...
              {[
                ...
                <label key={key} className='flex items-center gap-2'>
                  <input
                    ...
                    checked={
                      profile.writingPatterns[
                        key as keyof typeof profile.writingPatterns
                      ] as boolean
                    }
                    onChange={e =>
                      onUpdate({
...
const SamplesTab: React.FC<{
  ...
  const [selectedType, setSelectedType] = useState<'resume' | 'cover_letter' | 'email' | 'social'>(
    'email'
  );
        ...
        <div className='space-y-4'>
          <div>
            ...
            <select
              value={selectedType}
              onChange={e =>
                setSelectedType(e.target.value as 'resume' | 'cover_letter' | 'email' | 'social')
              }
              ...
              <div key={index} className='p-4 border border-gray-200 rounded-lg'>
                <div className='flex items-center justify-between mb-2'>
                  <div className='flex items-center gap-2'>
                    ...
                    {sample.analyzed && <CheckCircle className='w-4 h-4 text-green-500' />}
                  </div>
                  <button
                    onClick={() => {
        ...
        <div className='prose max-w-none'>
          ...
          <div className='bg-gray-50 rounded-lg p-4 mb-4'>
            <p className='text-sm text-gray-800'>
              <strong>Your writing style is {profile.tone}</strong> with a formality level of{' '}
              {profile.formality}/10. You tend to write with{' '}
              {profile.confidence >= 7 ? 'high' : profile.confidence >= 5 ? 'moderate' : 'low'}{' '}
              confidence and{' '}
              {profile.enthusiasm >= 7 ? 'high' : profile.enthusiasm >= 5 ? 'moderate' : 'low'}{' '}
              enthusiasm. Your vocabulary is typically {profile.vocabulary}, and you prefer{' '}
              {profile.sentenceStructure} sentences.
            </p>
          </div>
          ...
          <ul className='text-sm text-gray-700 space-y-1'>
            ...
            <li>
              •{' '}
              {profile.writingPatterns.paragraphLength.charAt(0).toUpperCase() +
                profile.writingPatterns.paragraphLength.slice(1)}{' '}
              paragraphs
            </li>
