...
test.describe('Job Search Flow - Visual QA', () => {
  ...
  test('should complete job search workflow with proper UI feedback', async () => {
    ...
    if (isLoginPage) {
      console.log(
        '❌ QA Issue: Landing directly on login page - should have guest/demo access for job search'
      );

      // Attempt to login or find demo access
      const demoButton = page
        .locator('button:has-text("Demo"), button:has-text("Guest"), button:has-text("Try")')
        .first();
    ...
    for (const selector of saveButtonSelectors) {
      ...
      if (await saveButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        if (buttonStyles.opacity === '0.5' || buttonStyles.cursor === 'not-allowed') {
          console.log(
            '⚠️ QA Issue: Save button may appear disabled (low opacity or not-allowed cursor)'
          );
        }
    ...
    if (savedJob) {
      ...
      if (!hasFeedback) {
        console.log(
          '⚠️ QA Issue: No visible feedback after saving job (toast, notification, etc.)'
        );
      }
    }
  ...
  test('should verify M3 design token compliance in job cards', async () => {
    ...
    const jobCards = page.locator(
      '[class*="Card"], [class*="card"], article, [data-testid*="job"]'
    );
    ...
    const hasOrganicRadius =
      m3Compliance.borderRadius.includes('px') && m3Compliance.borderRadius.split(' ').length > 1;

    if (!hasOrganicRadius) {
      console.log(
        '⚠️ QA Issue: Card not using M3 [DEPRECATED_STYLE] border radius (should be asymmetric like pebble)'
      );
    } else {
