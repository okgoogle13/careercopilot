...
• Collaborative and innovative work environment
    ...
    const kscResultsContainer = page.locator('[data-testid="ksc-results"], .ksc-results, .generated-responses').first();
    ...
    await expect(async () => {
      const generatedContent = await page.locator('text=/Demonstrated|Applied|Managed|Led|Implemented|Achieved/i').first();
    ...
    const kscPatterns = page.locator('text=/Demonstrated.*|Applied.*|Implemented.*|Managed.*|Led.*/');
  ...
  test('KSC generation with minimal job description', async ({ page }) => {
    ...
    const jobDescriptionTextarea = page.locator('textarea').first();
    await jobDescriptionTextarea.fill('Python developer position requiring 3+ years experience and strong problem-solving skills.');
  ...
  test('responsive design - mobile viewport', async ({ page }) => {
    ...
    await jobDescriptionTextarea.fill('Mobile test: Software engineer position with React experience.');
