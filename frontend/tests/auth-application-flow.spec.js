...
test.describe('Critical Flow: Auth → Create Application → Verify Database', () => {
    ...
    test.beforeAll(async () => {
        ...
        try {
            ...
            throw new Error('Test user authentication failed. Ensure TEST_USER_EMAIL and TEST_USER_PASSWORD are set.');
        }
    });
    ...
    test('should complete full authentication and application creation flow', async ({ page }) => {
        ...
        await test.step('Log in with test user credentials', async () => {
            ...
            const userProfile = page.locator('[data-testid="user-profile"], .user-profile, [aria-label*="user"]');
        ...
        await test.step('Navigate to Application Tracker', async () => {
            ...
            // Verify page header
            await expect(page.locator('h1, h2').filter({ hasText: /application/i }).first()).toBeVisible();
        ...
        await test.step('Create a new job application', async () => {
            ...
            const addButton = page.locator('button:has-text("Add New Application"), button:has-text("+ Add")');
            await addButton.first().click();

            // Wait for application form/modal to appear
            await page.waitForSelector('input[name="jobTitle"], input[placeholder*="Job Title"]', { timeout: 5000 });
            ...
            try {
                await page.fill('input[name="location"], input[placeholder*="Location"]', 'Remote', { timeout: 2000 });
                await page.fill('input[name="salary"], input[placeholder*="Salary"]', '$100k - $120k', { timeout: 2000 });
            } catch (e) {
        ...
        await test.step('Verify application appears in the tracker', async () => {
            ...
            const isVisible = await applicationCard.first().isVisible().catch(() => false) ||
                await applicationText.first().isVisible().catch(() => false);
        ...
        await test.step('Verify application exists in Firestore', async () => {
            ...
            const q = query(appsRef, where('jobTitle', '>=', 'E2E Test'), where('jobTitle', '<=', 'E2E Test\uf8ff'));
        ...
        await test.step('Verify UI and database data match', async () => {
            ...
            // Get UI data
            const uiJobTitle = await page.locator('[data-testid="application-card"]')
    ...
    test('should validate application form inputs', async ({ page }) => {
        ...
        await test.step('Attempt to create application with empty fields', async () => {
            ...
            const isErrorVisible = await validationError.first().isVisible({ timeout: 3000 }).catch(() => false);
