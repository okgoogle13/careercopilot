...
def has_column(table_name: str, column_name: str) -> bool:
    ...
    conn = op.get_bind()
    result = conn.execute(sa.text(f"PRAGMA table_info({table_name})")).fetchall()
...
def table_has_index(table_name: str, index_name: str) -> bool:
    ...
    conn = op.get_bind()
    result = conn.execute(sa.text(f"PRAGMA index_list({table_name})")).fetchall()
...
def upgrade() -> None:
    ...
    if not has_table("applications"):
        op.create_table(
            ...
            sa.Column(
                "job_id", sa.String(36), sa.ForeignKey("jobs.id"), nullable=False, index=True
            ),
        ...
        op.create_index(
            "ix_applications_interview_scheduled", "applications", ["interview_scheduled"]
        )
        ...
        with op.batch_alter_table("ai_interactions") as batch_op:
            ...
            if not has_column("ai_interactions", "metadata"):
                ...
                batch_op.alter_column(
                    "model_used", existing_type=sa.String(100), nullable=False, server_default=""
                )
            ...
            if has_column("ai_interactions", "success"):
                ...
                batch_op.alter_column(
                    "tokens_used", existing_type=sa.INTEGER(), nullable=False, server_default="0"
                )
            if has_column("ai_interactions", "success"):
                batch_op.alter_column(
                    "success", existing_type=sa.BOOLEAN(), nullable=False, server_default="1"
                )
    ...
    if has_table("cache"):
        with op.batch_alter_table("cache") as batch_op:
            if not has_column("cache", "metadata"):
                batch_op.add_column(sa.Column("metadata", JSON, nullable=True))
            if has_column("cache", "expires_at") and not table_has_index(
                "cache", "idx_cache_expires"
            ):
                batch_op.create_index("idx_cache_expires", ["expires_at"])
            if has_column("cache", "operation_type") and not table_has_index(
                "cache", "idx_cache_operation_type"
            ):
    ...
    if has_table("jobs"):
        with op.batch_alter_table("jobs") as batch_op:
            ...
            if not has_column("jobs", "job_type"):
                ...
                batch_op.add_column(
                    sa.Column("remote_ok", sa.Boolean, nullable=True, server_default=sa.text("0"))
                )
                batch_op.add_column(sa.Column("experience_level", sa.String(50), nullable=True))
            if not has_column("jobs", "remote_ok"):
                batch_op.add_column(
                    sa.Column("remote_ok", sa.Boolean, nullable=True, server_default="0")
                )
