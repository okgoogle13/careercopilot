...
async def analyze_resume(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"An unexpected error occurred during resume analysis: {str(e)}")
...
async def analyze_job_description(
    ...
    try:
        ...
        analysis_id = str(uuid.uuid4())
        analysis_ref = db.collection("users").document(user['uid']).collection("job_analyses").document(analysis_id)
        analysis_data = {
            "id": analysis_id,
            "createdAt": SERVER_TIMESTAMP,
            "jobDescription": request.job_description,
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"An unexpected error occurred during job analysis: {str(e)}")
