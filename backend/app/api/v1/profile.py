...
async def generate_and_save_voice_profile(
    ...
    if mock_db:
        mock_db.collection("users").document(user_id).set({"voice_profile": voice_profile_data}, merge=True)
...
async def generate_and_save_voice_profile(
    ...
    # Save to Firestore (already done in flow, but for test compatibility)
    # Optionally, you could re-fetch and return the saved profile
    return voice_profile_data
