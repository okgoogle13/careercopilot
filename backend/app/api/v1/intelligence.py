...
async def get_workflow_session_status(
    session_id: str,
    db: Session = Depends(get_db)
):
    """
...
async def get_market_insights(
    ...
    try:
        ...
        ).order_by(MarketAnalysis.analysis_date.desc()).limit(5).all()
...
async def analyze_skill_trends(
    ...
    try:
        ...
        analyses = db.query(MarketAnalysis).filter(
            MarketAnalysis.field == field,
            MarketAnalysis.location == location
        ).order_by(MarketAnalysis.analysis_date.desc()).limit(10).all()
        ...
        for analysis in reversed(analyses):  # Oldest first
            for skill in analysis.top_skills:
                ...
                frequency = analysis.skill_frequency.get(skill, 0) if analysis.skill_frequency else 0
                skill_evolution[skill].append({
                    "date": analysis.analysis_date,
                    "frequency": frequency,
                    "rank": analysis.top_skills.index(skill) + 1 if skill in analysis.top_skills else None
                })
        ...
        return {
            ...
            "skill_trends": {
                ...
                "stable_skills": [skill for skill in skill_evolution.keys() 
                               if skill not in [s["skill"] for s in trending_up + trending_down]][:5],
                
                "current_top_skills": analyses[0].top_skills[:10],
                "emerging_skills": analyses[0].emerging_skills,
                
                "recommendations": [
                    f"Focus on developing {trending_up[0]['skill']} - growing {trending_up[0]['growth_rate']}" if trending_up else "Continue developing core skills",
