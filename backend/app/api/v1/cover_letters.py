...
async def generate_cover_letter(
    ...
    try:
        ...
        return {
            "document": new_doc_data,
            "analysis": cover_letter_result
        }
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"An error occurred while generating cover letter: {str(e)}")

@router.post("/optimize")
async def optimize_cover_letter(
    request: CoverLetterOptimizeRequest,
    user: dict = Depends(get_current_user)
):
    """
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"An error occurred while optimizing cover letter: {str(e)}")
...
async def generate_cover_letter_templates(
    ...
    try:
        ...
        templates_ref = db.collection("users").document(uid).collection("cover_letter_templates").document(templates_id)
        templates_data = {
            "id": templates_id,
            "createdAt": SERVER_TIMESTAMP,
            "industry": request.industry,
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"An error occurred while generating templates: {str(e)}")