...
async def daily_job_discovery(
    ...
    try:
        ...
        if result.get("success"):
            ...
            session.total_duration_ms = int((datetime.utcnow() - session.started_at).total_seconds() * 1000)
            db.commit()
            
            logger.info(f"Job discovery completed for user {request.user_id}: {len(stored_jobs)} new jobs")
...
async def salary_analysis(
    ...
    try:
        ...
        ai_interaction.response_time_ms = int((datetime.utcnow() - start_time).total_seconds() * 1000)
    ...
    try:
        ...
        if result.get("success"):
            ...
            application = Application(
                ...
                job_id=job.id,
                status="prepared",
                cover_letter=result.get("application_materials", {}).get("cover_letter", {}).get("content"),
                email_application=result.get("application_materials", {}).get("email_application", {}).get("content"),
                follow_up_email=result.get("application_materials", {}).get("follow_up_email", {}).get("content"),
                interview_thank_you=result.get("application_materials", {}).get("interview_thank_you", {}).get("content"),
...
async def weekly_review(
    ...
    try:
        ...
        applications_this_week = db.query(Application).filter(
            ...
            Application.last_updated >= week_ago
        ).count()
        
        jobs_discovered = db.query(Job).filter(
            Job.user_id == user_id,
