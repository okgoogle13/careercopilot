...
async def get_available_models(
    ...
    try:
        ...
        return {
            "timestamp": datetime.utcnow().isoformat(),
            "filters": {"provider": provider, "model_type": model_type, "service": service},
            "models": models,
...
async def get_ai_services(
    enabled_only: bool = Query(False, description="Return only enabled services")
):
    """
    ...
    try:
        ...
        all_services = (
            ...
            if enabled_only
            else list(config_manager.services.values())
        )
    ...
    try:
        ...
        return {
            ...
            "operations_by_type": {
                ...
                if name.startswith("ai_operation_")
                and not name.endswith("_total")
                and not name.endswith("_cached")
            },
...
async def get_cost_analysis(
    ...
    try:
        ...
        for service_name, service_config in config_manager.services.items():
            ...
            service_operations = metrics_summary.get("counters", {}).get(
                f"ai_operation_{service_name}", 0
            )
                ...
                if primary_model:
                    ...
                    service_costs[service_name] = {
                        ...
                        "budget_utilization_percent": (
                            service_cost / (service_config.cost_budget_daily * days)
                        )
                        * 100,
                    }
        ...
        return {
            ...
            "avg_cost_per_operation": round(
                ...
                / max(sum(sc["operations"] for sc in service_costs.values()), 1),
                6,
            ),
