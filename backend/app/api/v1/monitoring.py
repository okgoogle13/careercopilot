    ...
    try:
        ...
        for op_name, op_metrics in performance_data.items():
            op_metrics['throughput_per_minute'] = _calculate_throughput(
                op_metrics.get('count', 0), 
                metrics.get('uptime_seconds', 1)
            )
...
async def get_dashboard_data():
    ...
    try:
        ...
        dashboard_data = {
            ...
            'ai_metrics': {
                'total_operations': metrics_summary.get('counters', {}).get('ai_operation_total', 0),
                'cached_operations': metrics_summary.get('counters', {}).get('ai_operation_cached_total', 0),
