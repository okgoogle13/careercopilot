...
from app.models.database import Job, MarketAnalysis, User
...
class JobMarketAnalyzer:
    ...
    async def analyze_market_trends(self, field: str, location: str, refresh_data: bool = False) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if not refresh_data:
                with get_db_session() as db:
                    recent_analysis = db.query(MarketAnalysis).filter(
            ...
            with get_db_session() as db:
                market_analysis = MarketAnalysis(
                    ...
                    average_salary=analysis_results.get("salary_insights", {}).get("average_salary"),
                    salary_range=analysis_results.get("salary_insights", {}).get("salary_range", {}),
                    top_skills=analysis_results.get("skill_insights", {}).get("top_skills", []),
                    emerging_skills=analysis_results.get("skill_insights", {}).get("emerging_skills", []),
                    skill_frequency=analysis_results.get("skill_insights", {}).get("skill_frequency", {}),
                    top_employers=analysis_results.get("employer_insights", {}).get("top_employers", []),
                    company_hiring_trends=analysis_results.get("employer_insights", {}).get("hiring_trends", {}),
    ...
    async def _perform_ml_analysis(self, job_data: List[Dict], field: str, location: str) -> Dict[str, Any]:
    ...
    async def _analyze_skills(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        skill_keywords = {
            ...
            "community_engagement": ["community engagement", "community outreach", "community development"],
            ...
            "crisis_intervention": ["crisis intervention", "crisis response", "emergency"],
        ...
        return {
            ...
            "emerging_skills": ["Digital Literacy", "Trauma-Informed Care", "Telehealth"],
    ...
    async def _forecast_demand(self, df: pd.DataFrame, field: str, location: str) -> Dict[str, Any]:
        ...
        forecast = {
            ...
            "short_term_outlook": "growing" if recent_trend > 0 else "declining" if recent_trend < 0 else "stable",
            "predicted_growth": f"{abs(recent_trend * 30):.0f} jobs/month" if recent_trend != 0 else "stable",
            "confidence": "medium",
            "factors": [
    ...
    async def _assess_competition(self, df: pd.DataFrame) -> str:
        ...
        job_count = len(df)
        unique_employers = df['company'].nunique()
    ...
    def _format_market_analysis(self, analysis: MarketAnalysis) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "data_freshness": "recent" if analysis.expires_at > datetime.utcnow() else "expired"
            },
...
class SkillMatchingEngine:
    ...
    async def calculate_job_match_score(self, user_profile: Dict[str, Any], job_description: str) -> Dict[str, Any]:
        ...
        try:
            ...
            skill_match = await self._calculate_skill_match(user_skills, job_analysis["required_skills"])
            experience_match = await self._calculate_experience_match(user_experience, job_analysis["experience_required"])
            background_match = await self._calculate_background_match(user_background, job_analysis["preferred_background"])
    ...
    async def _analyze_job_requirements(self, job_description: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "education_required": "Bachelor's degree" if "bachelor" in job_lower else "Certificate"
        }
    
    async def _calculate_skill_match(self, user_skills: List[str], required_skills: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            "missing_skills": [skill for skill in required_skills_lower if skill not in matched_skills],
            "details": f"Matched {len(matched_skills)}/{len(required_skills)} required skills"
        }
    
    async def _calculate_experience_match(self, user_experience: int, required_experience: int) -> Dict[str, Any]:
    ...
    async def _calculate_background_match(self, user_background: str, preferred_backgrounds: List[str]) -> Dict[str, Any]:
        ...
        return {
            ...
            ] if user_bg_lower == "finance" else ["Professional experience"],
            "details": f"{user_background} background brings valuable transferable skills"
        }
    
    async def _generate_improvement_recommendations(self, skill_match, experience_match, background_match) -> List[str]:
