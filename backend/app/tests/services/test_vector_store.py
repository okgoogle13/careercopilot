...
def test_add_artifact(vector_store, sample_artifact, mock_genai):
    ...
    with patch("app.services.vector_store.get_db_session") as mock_get_db, patch(
        "app.services.vector_store.DocumentEmbedding", FakeDocumentEmbedding
    ):
...
def test_query_similar_basic(vector_store, mock_genai):
    ...
    with patch("app.services.vector_store.get_db_session") as mock_get_db, patch(
        ...
        mock_result_doc = SimpleNamespace(
            ...
            user_id="test-user",
            content="Found text",
            metadata_json={"source_type": "resume"}
        )
