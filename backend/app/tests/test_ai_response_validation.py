...
class TestAIFlowIntegration:
    ...
    def test_extract_validated_data_error(self):
        ...
        validation_result = ValidationResult(is_valid=False, error_message="Test error")
...
class TestEdgeCases:
    ...
    def test_numeric_boundary_validation(self):
        ...
        valid_semantic = json.dumps(
            {"similarity_score": 0.0, "explanation": "Minimum score test"}  # Minimum
        )
        result = self.validator.validate_response(valid_semantic, "semantic_analysis")
        assert result.is_valid

        valid_semantic = json.dumps(
            {"similarity_score": 100.0, "explanation": "Maximum score test"}  # Maximum
        )
        result = self.validator.validate_response(valid_semantic, "semantic_analysis")
        assert result.is_valid
