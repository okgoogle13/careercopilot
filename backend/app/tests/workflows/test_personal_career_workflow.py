...
def workflow():
    ...
    with (
        patch("app.workflows.personal_career_workflow.get_personal_config") as mock_config,
        patch("app.workflows.personal_career_workflow.get_ai_client") as mock_ai_client,
        patch(
            "app.workflows.personal_career_workflow.get_ai_prompt_builder"
        ) as mock_prompt_builder,
        patch(
            "app.workflows.personal_career_workflow.get_template_service"
...
async def test_salary_intelligence(workflow):
    ...
    with patch(
        "app.workflows.personal_career_workflow.web_search", new_callable=AsyncMock
    ) as mock_search:
        ...
        workflow.ai_prompt_builder.generate_ai_response = AsyncMock(
...
async def test_apply_to_job(workflow):
    ...
    with (
        patch.object(workflow, "quick_company_research", new_callable=AsyncMock) as mock_research,
        ...
        workflow.template_service.generate_application_materials = AsyncMock(
...
async def test_generate_cover_letter_template(workflow):
    ...
    mock_template = MagicMock(
        content="CL Body", placeholders={}, customization_tips=[], generated_at="now"
    )
    workflow.template_service.generate_template = AsyncMock(return_value=mock_template)
...
@pytest.mark.asyncio
async def test_apply_to_job_returns_error_when_research_fails(workflow):
    """The workflow should short-circuit when company research fails."""
    ...
    assert result == {"success": False, "error": "Company research failed"}


@pytest.mark.asyncio
async def test_generate_email_template_unknown_type(workflow):
    result = await workflow.generate_email_template("unknown")
    assert result["success"] is False
    assert "Unknown template type" in result["error"]


@pytest.mark.asyncio
async def test_generate_email_template_handles_service_exception(workflow):
    workflow.template_service.generate_template = AsyncMock(
        side_effect=RuntimeError("template failed")
    )
    result = await workflow.generate_email_template("application", "Role", "Org")
    assert result == {"success": False, "error": "template failed"}


@pytest.mark.asyncio
async def test_generate_cover_letter_template_handles_service_exception(workflow):
    workflow.template_service.generate_template = AsyncMock(
        side_effect=RuntimeError("template failed")
    )
    result = await workflow.generate_cover_letter_template("Role", "Org")
    assert result == {"success": False, "error": "template failed"}


@pytest.mark.asyncio
async def test_quick_company_research_error_path(workflow):
    workflow.ai_prompt_builder.generate_ai_response = AsyncMock(
        side_effect=RuntimeError("ai unavailable")
    )
    result = await workflow.quick_company_research("http://example.com")
    assert result == {"success": False, "error": "ai unavailable"}


@pytest.mark.asyncio
async def test_daily_job_discovery_handles_config_errors(workflow):
    workflow.config.salary_range = None
    result = await workflow.daily_job_discovery()
    assert result["success"] is False
