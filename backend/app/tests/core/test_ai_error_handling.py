...
from app.core.ai_error_handling import (
    AIOperationHandler, AIError, AIErrorType, RetryConfig,
    with_ai_error_handling, validate_ai_response, create_user_friendly_error
)
...
class TestAIOperationHandler:
    ...
    def test_should_retry_logic(self):
        ...
        # Should retry
        assert handler.should_retry(AIErrorType.RATE_LIMIT) == True
        assert handler.should_retry(AIErrorType.TIMEOUT) == True
        assert handler.should_retry(AIErrorType.SERVICE_UNAVAILABLE) == True
        assert handler.should_retry(AIErrorType.UNKNOWN) == True
