...
async def test_analyze_resume_disabled(document_analysis_service):
    ...
    result = await document_analysis_service.analyze_resume(
        "Valid long text content goes here for the test."
    )
...
async def test_analyze_job_description_invalid_input(document_analysis_service):
    ...
    with pytest.raises(
        ValueError, match="Job description text is too short for meaningful analysis"
    ):
...
async def test_analyze_document_generic_disabled(document_analysis_service):
    ...
    with pytest.raises(AIError) as exc_info:
        await document_analysis_service.analyze_document_generic(
...
async def test_extract_skills_resume(document_analysis_service):
    ...
    with patch.object(document_analysis_service, "is_available", return_value=True):
        with patch.object(
            document_analysis_service, "analyze_resume", new=AsyncMock(return_value=expected_resume)
        ):
...
async def test_extract_skills_generic(document_analysis_service):
    ...
    with patch.object(document_analysis_service, "is_available", return_value=True):
        with patch.object(
            document_analysis_service,
            "analyze_document_generic",
            new=AsyncMock(return_value=expected_generic),
        ):
...
async def test_extract_skills_error(document_analysis_service):
    ...
    with patch.object(document_analysis_service, "is_available", return_value=True):
        with patch.object(
            document_analysis_service, "analyze_resume", side_effect=Exception("Failed")
        ):
...
async def test_convenience_functions():
    ...
    with patch(
        "app.ai.document_analysis_service.get_document_analysis_service", return_value=mock_service
    ):
