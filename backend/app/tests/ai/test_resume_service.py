...
async def mock_process_document():
    ...
    mock = AsyncMock()
    with patch("app.ai.resume_service.process_document", new=mock):
        yield mock
...
async def test_analyze_resume_sanitize_input(
    ...
    assert mock_process_document.call_args[0][0] == "This is a sample resume."


def test_resume_analysis_service_init_with_config():
...
def test_resume_analysis_service_init_without_config():
    """Test ResumeAnalysisService initialization without config."""
    service = ResumeAnalysisService()
    assert service.config["model"] == settings.ai_model
