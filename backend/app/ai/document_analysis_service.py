...
class JobDescriptionAnalysisResult(BaseModel):
    ...
    salary_range: SalaryRange | None = Field(None, description="Salary information")
...
class DocumentAnalysisService(BaseAIService):
    ...
    async def analyze_resume(
        ...
        try:
            ...
            error_msg = f"Failed to analyze resume: {e!s}"
    ...
    async def analyze_job_description(
        ...
        try:
            ...
            error_msg = f"Failed to analyze job description: {e!s}"
            logger.error(error_msg, exc_info=True)
    ...
    async def analyze_document_generic(
        ...
        **template_params,
        ...
        try:
            ...
            error_msg = f"Failed to analyze document with template {template_id}: {e!s}"
            logger.error(error_msg, exc_info=True)
            raise
...
# Global instance
_document_analysis_service: DocumentAnalysisService | None = None
