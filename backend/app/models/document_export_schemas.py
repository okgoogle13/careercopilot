...
class DocumentExportRequest(BaseModel):
    ...
    expiration_hours: float = Field(
        default=24.0, description="Signed URL expiration time in hours (1-168)", ge=1, le=168
    )
...
class CoverLetterExportRequest(DocumentExportRequest):
    ...
    job_title: str = Field(
        ..., description="Job title for export context", min_length=1, max_length=200
    )
    company_name: str | None = Field(
        default=None, description="Company name (optional)", max_length=200
    )
...
class ResumeExportRequest(DocumentExportRequest):
    ...
    job_title: str = Field(
        ..., description="Target job title for personalization", min_length=1, max_length=200
    )
...
class ApplicationPackageExportRequest(DocumentExportRequest):
    ...
    job_id: str = Field(..., description="Job ID for export context", min_length=1)
    include_files: bool = Field(default=True, description="Include file attachments in ZIP export")
...
class BatchExportRequest(BaseModel):
    ...
    format: str = Field(
        default="json", description="Export format", pattern="^(json|pdf|docx|txt|zip)$"
    )
    expiration_hours: float = Field(
        default=24.0, description="Signed URL expiration time in hours", ge=1, le=168
    )
...
class DocumentExportResponse(BaseModel):
    ...
    file_format: str = Field(description="Format of exported file: json, pdf, docx, txt, zip")
    download_url: str = Field(
        description="Signed URL for downloading exported document (valid for specified hours)"
    )
    file_size_bytes: int = Field(ge=0, description="Size of exported file in bytes")
    storage_path: str = Field(
        description="Cloud Storage path (gs://bucket/path) for audit and retention"
    )
    expires_at: str = Field(description="ISO 8601 timestamp when signed URL expires")
...
class CoverLetterExportResponse(DocumentExportResponse):
    ...
    document_type: str = Field(
        default="cover_letter", description="Document type is always cover_letter"
    )
...
class ResumeExportResponse(DocumentExportResponse):
    ...
    document_type: str = Field(default="resume", description="Document type is always resume")
...
class KSCResponseExportResponse(DocumentExportResponse):
    ...
    document_type: str = Field(
        default="ksc_response", description="Document type is always ksc_response"
    )
...
class ApplicationPackageExportResponse(DocumentExportResponse):
    ...
    document_type: str = Field(
        default="application_package", description="Document type is always application_package"
    )
...
class BatchExportResponse(BaseModel):
    ...
    success: bool = Field(description="Whether all exports were successful")
    exports: list[DocumentExportResponse] = Field(
        description="List of export results for each document type"
    )
    total_size_bytes: int = Field(ge=0, description="Total size of all exported files")
    message: str = Field(description="Success or error summary message")
...
class FileDownloadMetadata(BaseModel):
    ...
    file_id: str = Field(description="Unique file identifier (from export operation)")
    file_name: str = Field(description="Original or suggested file name")
    ...
    expires_at: datetime = Field(description="Timestamp when file will expire")
    storage_path: str = Field(description="Cloud Storage path for auditing")


class DownloadUrlRequest(BaseModel):
    """Request for download URL (if needed in future)"""

    file_id: str = Field(description="File ID from previous export operation")
    expiration_hours: float = Field(
        default=24.0, description="Requested URL expiration", ge=1, le=168
    )
...
class DownloadUrlResponse(BaseModel):
    ...
    download_url: str = Field(description="Signed URL for file download")
    expires_at: str = Field(description="ISO 8601 timestamp when URL expires")
    metadata: FileDownloadMetadata = Field(description="File metadata")
    message: str = Field(description="Success or error message")
...
class CoverLetterResponseWithExport(BaseModel):
    ...
    export_options: dict[str, str] | None = Field(
        default=None, description="Available export formats and endpoints to retrieve signed URLs"
    )
...
class ResumeResponseWithExport(BaseModel):
    ...
    success: bool = Field(description="Whether generation was successful")
    resume: dict[str, Any] | None = Field(description="Generated resume data (for immediate use)")
    message: str = Field(description="Success or error message")
    processingTimeSeconds: float = Field(description="Total processing time")
    export_options: dict[str, str] | None = Field(
        default=None, description="Available export formats and endpoints"
    )
