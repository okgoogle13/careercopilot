...
def _generate_html_from_content(
    title: str,
    content_html: str,
...
def render_cover_letter_pdf(
    ...
    full_html = _generate_html_from_content(
        title="Cover Letter",
        content_html=f"{header_html}{body_html}",
        theme_id=theme_id
    )
...
def render_resume_pdf(
    ...
    contact_parts = []
    if basics.get("email"): contact_parts.append(basics["email"])
    if basics.get("phone"): contact_parts.append(basics["phone"])
    if basics.get("location"): contact_parts.append(basics["location"])
    if contact_parts:
        content_parts.append(f"<div class='contact'>{' | '.join(contact_parts)}</div>")

    if sections.get("summary"):
        content_parts.append(f"<h2>Professional Summary</h2><div class='section'>{sections['summary']}</div>")
    ...
    if sections.get("work"):
        ...
        for job in sections["work"]:
            ...
            content_parts.append(f"<div class='section'><span class='job-title'>{job.get('role', 'Role')}</span> at {job.get('company', 'Company')} {dates}")
