...
from .logging_config import RequestContextLogger, request_id_context, user_id_context
...
class RequestMonitoringMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        self.collector.increment_counter('http_requests_total', labels={
            'method': request.method,
        ...
        try:
            ...
            self.collector.increment_counter('http_responses_total', labels={
                'method': request.method,
                'path': self._normalize_path(request.url.path),
            ...
            # Track error metrics
            self.collector.increment_counter('http_requests_errors_total', labels={
                'method': request.method,
                'path': self._normalize_path(request.url.path),
    ...
    def _track_user_action(self, request: Request, user_id: str):
        ...
        action_mapping = {
            ...
            ('POST', '/api/v1/profile/generate-voice-profile'): 'voice_profile_generation',
...
def add_monitoring_middleware(app: FastAPI, config: Dict[str, Any] = None):
    ...
    app.add_middleware(
        ...
        enable_alerting=config.get('enable_alerting', False)
    )
