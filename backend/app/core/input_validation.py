...
class InputSanitizer:
    ...
    def sanitize_text_input(cls, text: str, max_length: Optional[int] = None) -> SanitizedInput:
        ...
        if original_length > max_len:
            raise InputValidationError(f"Input too long: {original_length} chars (limit: {max_len})")
    ...
    def sanitize_dict_input(cls, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, str):
                try:
                    sanitized_input = cls.sanitize_text_input(value, max_length=cls.MAX_PROMPT_LENGTH)
