...
class AIOperationHandler:
    ...
    async def execute_with_retry(self, operation: Callable, *args, **kwargs) -> Any:
        ...
        for attempt in range(1, self.retry_config.max_attempts + 1):
            try:
                logger.info(f"Executing AI operation, attempt {attempt}/{self.retry_config.max_attempts}")
        ...
        raise AIError(
            message=f"Operation failed after {self.retry_config.max_attempts} attempts: {str(last_error)}",
