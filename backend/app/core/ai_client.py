...
from .ai_config import AIConfigManager, AIProvider, ModelConfig, AIServiceConfig, get_ai_config
from .monitoring import track_ai_usage, track_error, get_metrics_collector, monitor_performance
from .cache_decorators import cached_ai_operation
...
class OpenAIClient(AIProviderClient):
    ...
    async def generate_text(self, request: AIRequest, model_config: ModelConfig) -> AIResponse:
        ...
        if request.functions:
            payload["tools"] = [{"type": "function", "function": func} for func in request.functions]
    ...
    async def generate_embeddings(self, texts: List[str], model_config: ModelConfig) -> List[List[float]]:
...
class AIClientManager:
    ...
    async def generate_text(self, request: AIRequest) -> AIResponse:
        ...
        models_to_try = [model_name] + [
            m for m in service_config.fallback_models 
            if m != model_name
        ]
        ...
        for attempt_model in models_to_try:
            try:
                ...
                if not client:
                    logger.warning(f"No client available for provider: {model_config.provider.value}")
                    continue
        ...
        raise Exception(f"All models failed for service {request.service_name}. Last error: {last_error}")
...
def setup_ai_client(config_manager: Optional[AIConfigManager] = None) -> AIClientManager:
