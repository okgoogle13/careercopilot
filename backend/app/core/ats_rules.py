...
FORMAT_RULES_PATH = Path(__file__).parent.parent.parent.parent / "ai" / "prompts" / "backend" / "format_rules.json"
...
def validate_template_schema(template: dict, doc_type: str) -> None:
    ...
    present_headings = {b.get("title", "").lower() for b in template.get("blocks", []) if b.get("kind") == "section"}
    
    # We do a loose check or exact check based on lowercase
    missing = set(h.lower() for h in required_headings) - present_headings
    
    if missing:
        raise ValueError(f"Missing required ATS headings for {doc_type}: {missing}. Present: {present_headings}")
