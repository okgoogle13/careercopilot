...
class AICache:
    ...
    def _generate_cache_key(self, operation_type: str, user_id: str, 
                          input_data: Any, **kwargs) -> str:
    ...
    def _create_cache_entry(self, key: str, operation_type: str, 
                           input_data: Any, result: Any, **kwargs) -> CacheEntry:
        """Create a cache entry with appropriate TTL"""
        config = self.CACHE_CONFIGS.get(operation_type, 
                                       CacheConfig(ttl_seconds=1800))  # Default 30 min
    ...
    async def get(self, operation_type: str, user_id: str, 
                  input_data: Any, **kwargs) -> Optional[Any]:
        """Retrieve cached result for an AI operation"""
        try:
            cache_key = self._generate_cache_key(operation_type, user_id, 
                                               input_data, **kwargs)
    ...
    async def set(self, operation_type: str, user_id: str, 
                  input_data: Any, result: Any, **kwargs) -> bool:
        ...
        try:
            ...
            cache_key = self._generate_cache_key(operation_type, user_id, 
                                               input_data, **kwargs)
            entry = self._create_cache_entry(cache_key, operation_type, 
                                           input_data, result, **kwargs)
            
            success = await self.backend.set(entry)
            if success:
                logger.info(f"Cache SET for {operation_type} (user: {user_id}, TTL: {config.ttl_seconds}s)")
    ...
    async def invalidate_user_cache(self, user_id: str, 
                                  operation_types: Optional[List[str]] = None) -> int:
        """Invalidate all cache entries for a user"""
        try:
            if operation_types is None:
                # Invalidate all operations that are configured to invalidate on user update
                operation_types = [
                    op_type for op_type, config in self.CACHE_CONFIGS.items()
