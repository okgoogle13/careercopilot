...
def add_cache_middleware(app: FastAPI):
    ...
    app.add_middleware(
        CacheMonitoringMiddleware, include_headers=os.getenv("ENV", "development") != "production"
    )

    app.add_middleware(
        CacheCleanupMiddleware, cleanup_interval=int(os.getenv("CACHE_CLEANUP_INTERVAL", "3600"))
    )
