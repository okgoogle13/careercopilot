...
from .cache import get_ai_cache, AICache
...
class CacheMonitoringMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if self.include_headers and os.getenv('ENV') != 'production':
            response.headers['X-Cache-Stats'] = f"hits:{self.request_stats['cache_hits']},misses:{self.request_stats['cache_misses']}"
    ...
    async def _handle_invalidation(self, request: Request):
        ...
        try:
            ...
            if operation_types:
                ...
                logger.info(f"Invalidated {invalidated} cache entries for user {user_id} after {request.method} {endpoint_path}")
