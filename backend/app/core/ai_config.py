...
class ModelConfig:
    ...
    rate_limit: Dict[str, int] = field(default_factory=lambda: {"requests_per_minute": 60, "tokens_per_minute": 10000})
...
class AIConfigManager:
    ...
    def load_configuration(self) -> None:
        ...
        try:
            ...
            self._loaded = True
            logger.info(f"AI configuration loaded: {len(self.models)} models, {len(self.services)} services")
    ...
    def validate_configuration(self) -> Dict[str, List[str]]:
        ...
        for service in self.services.values():
            if service.primary_model not in self.models:
                issues['errors'].append(f"Service '{service.service_name}' uses unknown primary model: {service.primary_model}")
            
            for fallback_model in service.fallback_models:
                if fallback_model not in self.models:
                    issues['warnings'].append(f"Service '{service.service_name}' uses unknown fallback model: {fallback_model}")
            
            if service.cost_budget_daily <= 0:
                issues['warnings'].append(f"Service '{service.service_name}' has no cost budget set")
        
        # Check model configurations
        for model in self.models.values():
            if model.max_tokens <= 0:
                issues['errors'].append(f"Model '{model.name}' has invalid max_tokens: {model.max_tokens}")
            
            if not (0 <= model.temperature <= 2):
                issues['warnings'].append(f"Model '{model.name}' has unusual temperature: {model.temperature}")
