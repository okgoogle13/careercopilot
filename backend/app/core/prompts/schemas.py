...
class RedundancyIssue(BaseModel):
    ...
    example_code: str | None = Field(
        default=None, description="Optional code snippet showing the improved version"
    )
...
class AuditSummary(BaseModel):
    ...
    blocker_summary: str | None = Field(
        default=None, description="One-line explanation if not deployment-ready"
    )
...
class CIAuditResponse(BaseModel):
    ...
    def get_priority_issues(self, top_n: int = 5) -> list[RedundancyIssue]:
        ...
        sorted_issues = sorted(
            self.redundancy_report, key=lambda x: severity_order.get(x.severity, 0), reverse=True
        )
    ...
    def to_markdown(self) -> str:
        ...
        # Summary section
        lines.append(
            f"**Deployment Ready**: {'✅ Yes' if self.summary.deployment_ready else '❌ No'}\n"
        )
        ...
        if self.redundancy_report:
            ...
            for redundancy in self.get_priority_issues():
                ...
                lines.append(
                    f"### {severity_emoji[redundancy.severity]} {redundancy.file} (Lines {redundancy.lines})"
                )
        ...
        blocks = [
            {
                ...
                "text": {
                    "type": "plain_text",
                    "text": (
                        "🚨 Quick Scan Results" if self.has_blockers() else "✅ Quick Scan Clear"
                    ),
                },
            }
        ]
