...
const FilterSection: React.FC<FilterSectionProps> = ({ title, icon: Icon, children, defaultOpen = false, badge }) => {
...
export const FilterPanel: React.FC<FilterPanelProps> = ({ onFiltersChange, onClose, isOpen = true }) => {
  ...
  const handleDocumentTypeChange = (typeId: string, checked: boolean) => {
    const newTypes = checked ? [...filters.documentTypes, typeId] : filters.documentTypes.filter((id) => id !== typeId);
  ...
  const handleStatusChange = (statusId: string, checked: boolean) => {
    const newStatus = checked ? [...filters.status, statusId] : filters.status.filter((id) => id !== statusId);
    updateFilters({ status: newStatus });
  };

  const handleCreatedByChange = (createdById: string, checked: boolean) => {
    const newCreatedBy = checked ? [...filters.createdBy, createdById] : filters.createdBy.filter((id) => id !== createdById);
  ...
  const clearFilter = (filterType: keyof FilterState, value?: string) => {
    switch (filterType) {
      case 'documentTypes':
        updateFilters({ documentTypes: value ? filters.documentTypes.filter((id) => id !== value) : [] });
        break;
      case 'tags':
  ...
  return (
    ...
    >
      ...
      <Box>
        ...
        <FilterSection title="Document Type" icon={FileText} badge={filters.documentTypes.length} defaultOpen={true}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, mt: 2 }}>
            {documentTypeOptions.map((option) => (
              <Box key={option.id} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <FormControlLabel
                  control={
                    <Checkbox
        ...
        <FilterSection title="Tags" icon={Tag} badge={filters.tags.length}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5, mt: 2, maxHeight: 192, overflowY: 'auto' }}>
        ...
        <FilterSection title="Sort Order" icon={SortAsc}>
          <Box sx={{ mt: 2 }}>
            ...
            >
              ...
              <FormControlLabel value="name-asc" control={<Radio size="small" />} label="Name A-Z" />
              <FormControlLabel value="name-desc" control={<Radio size="small" />} label="Name Z-A" />
              <FormControlLabel value="size" control={<Radio size="small" />} label="File Size" />
              <FormControlLabel value="type" control={<Radio size="small" />} label="Document Type" />
            </RadioGroup>
          </Box>
        </FilterSection>
      </Box>
      ...
      >
        <Box sx={{ display: 'flex', gap: 1 }}>
          <Button variant="outlined" size="small" onClick={clearAllFilters} disabled={!hasActiveFilters} sx={{ flex: 1 }}>
            Clear All
          </Button>
          <Button
            variant="aurora"
