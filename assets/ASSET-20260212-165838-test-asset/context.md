# Asset ASSET-20260212-165838: Test-Asset

## Narrative

This kr-motifs asset exemplifies kerala-rage kr-solidarity aesthetics through strategic
placement of Australian endemic flora. The composition follows biological asymmetry principles,
creating a "found" rather than "built" aesthetic.

## kr-motifs

- Wattle (iconic Australian acacia)
- Leaf structures (native [DEPRECATED_STYLE] form)
- [DEPRECATED_STYLE] density zones (theatrical void + [DEPRECATED_STYLE] central)

## Mode Context

**kr-dark**: Warm, emotional interpretation for [DEPRECATED_MODE] (user-facing)
**kr-dark**: Clinical, analytical interpretation for [DEPRECATED_MODE] (tools)

## Purpose

Suitable for backgrounds, hero sections, or decorative elements in dashboard layouts.
Use with opacity 0.65-0.85 for [DEPRECATED_MODE], 0.05-0.20 for [DEPRECATED_MODE].

## Compliance Score

**Overall**: 75/100
**Decision**: REGENERATE ⚠️

